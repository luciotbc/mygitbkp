#!/usr/bin/env bash
# Ref: https://github.com/angristan/gitlab-repo-dl

# ./cloneall.sh group xxxxxxxx
...
if [ "$1" == "group" ]; then
    ...
    REPO_SSH_URLS=$(curl -s "$GITLAB_URL/api/v4/groups/$GROUP_NAME/projects?private_token=$GITLAB_TOKEN&include_subgroups=true&order_by=last_activity_at&per_page=999&per_page=999" | jq '.[] | .ssh_url_to_repo' | sed 's/"//g')
